...
> ℹ️ Telegram Bot 之間無法互相傳送或接收訊息。多 Bot「協作」的實現方式為：由人類使用者在不同 Bot 間協調，或在單一 Bot 內使用 `spawn_agent` 讓不同模型並行處理各自的子任務。
...
| 使用情境 | 建議方式 |
|----------|----------|
| 不同日常話題，只需隔離對話 | Topics |
| 需要進行 git 操作的開發專案 | 子代理 Bot（`/new_agent`） |
| 同一專案內的並行子任務 | `spawn_agent` 工具 |
| 群組中多個專業 Bot 各司其職 | 多個子代理 Bot，由人類協調 |
