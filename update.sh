...
if [[ "$LOCAL_VER" == "$REMOTE_VER" && "$LOCAL_VER" != "unknown" ]]; then
    printf "\n  ${G}✨ 已是最新版本！${NC}\n"
    printf "  如需强制更新，请运行: ${DIM}bash update.sh --force${NC}\n\n"
    [[ "${1:-}" != "--force" ]] && exit 0
fi

echo ""
printf "  即将更新以下核心文件（${Y}config.json / 自定义 tools / memory 不受影响${NC}）:\n"
printf "  agent.py  bot.py  main.py  tools_builtin.py  requirements.txt  update.sh  hydrabot  VERSION\n\n"
...
chmod +x update.sh hydrabot 2>/dev/null || true
